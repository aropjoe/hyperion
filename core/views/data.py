...
def performance_metrics(request, data_id):
    ...
    latency_data = [x["latency"] for x in server_logs][:20]
    response_times = [x["response_time"] for x in server_logs][:20]
...
def subscription_analytics(request, data_id):
    ...
    avg_rev = average_revenue(subscriptions_data, total_revenue)
    print(avg_rev)
    revenue_tier = revenue_by_tier(subscriptions_data)

    r_tiers, r_revenue = revenue_by_tier_array(revenue_tier)
